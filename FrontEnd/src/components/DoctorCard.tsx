...
export default function UsersDetail() {
  ...
  // --- LOGIC: Parse Specializations (Based on your reference) ---
  const specializationList = useMemo(() => {
    ...
    return [String(specs)];
  }, [user?.doctor_profile?.specializations]);
                            ...
                            <div className="sm:col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {specializationList.map((spec, index) => (
                                        <span 
                                            key={index} 
                                            className="px-3 py-1.5 bg-white border border-blue-200 text-blue-700 rounded-lg text-sm font-bold shadow-sm"
                                        >
                                            {spec}
                                        </span>
                                    ))}
